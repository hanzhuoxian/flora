...
# Example:  util::wait_for_success 120 5 "floractl get nodes|grep localhost"
# arguments: wait time, sleep time, shell command
# returns 0 if the shell command get output, 1 otherwise.
function util::wait_for_success(){
  local wait_time="$1"
  local sleep_time="$2"
  local cmd="$3"
...
function util::cleanup-temp-dir() {
  rm -rf "${TEMP_DIR}"
}
...
# Vars set:
#   TEMP_DIR
function util::ensure-temp-dir() {
  if [[ -z ${TEMP_DIR-} ]]; then
    TEMP_DIR=$(mktemp -d 2>/dev/null || mktemp -d -t florarnetes.XXXXXX)
...
function util::find-binary-for-platform() {
  local -r lookfor="$1"
  local -r platform="$2"
  local locations=(
    "${ROOT_DIR}/_output/bin/${lookfor}"
    "${ROOT_DIR}/_output/${platform}/${lookfor}"
    "${ROOT_DIR}/_output/local/bin/${platform}/${lookfor}"
    "${ROOT_DIR}/_output/platforms/${platform}/${lookfor}"
  )
...
# $ROOT_DIR must be set
function util::find-binary() {
  util::find-binary-for-platform "$1" "$(util::host_platform)"
}

# Run all known doc generators (today gendocs and genman for ctl)
# $1 is the directory to put those generated documents
function util::gen-docs() {
  local dest="$1"
  ...
  mkdir -p "${dest}/docs/guide/en-US/cmd/floractl/"
  "${gendocs}" "${dest}/docs/guide/en-US/cmd/floractl/"

  mkdir -p "${dest}/docs/guide/en-US/cmd/"
  "${genfloradocs}" "${dest}/docs/guide/en-US/cmd/" "flora-apiserver"
  "${genfloradocs}" "${dest}/docs/guide/en-US/cmd/floractl" "floractl"

  mkdir -p "${dest}/docs/man/man1/"
  "${genman}" "${dest}/docs/man/man1/" "flora-apiserver"
  "${genman}" "${dest}/docs/man/man1/" "floractl"

  mkdir -p "${dest}/docs/guide/en-US/yaml/floractl/"
  "${genyaml}" "${dest}/docs/guide/en-US/yaml/floractl/"
...
# Removes previously generated docs-- we don't want to check them in. $ROOT_DIR
# must be set.
function util::remove-gen-docs() {
...
function util::git_upstream_remote_name() {
  git remote -v | grep fetch |\
    grep -E 'github.com[/:]hanzhuoxian/flora' |\
    head -n 1 | awk '{print $1}'
}

# Exits script if working directory is dirty. If it's run interactively in the terminal
# the user can commit changes in a second terminal. This script will wait.
function util::ensure_clean_working_dir() {
...
# creates a self-contained floraconfig: args are sudo, dest-dir, ca file, host, port, client id, token(optional)
function util::write_client_floraconfig {
    local sudo=$1
    local dest_dir=$2
    local ca_file=$3
    local api_host=$4
    ...
    local client_id=$6
    local token=${7:-}
    cat <<EOF | ${sudo} tee "${dest_dir}"/"${client_id}".floraconfig > /dev/null
apiVersion: v1
kind: Config
clusters:
  - cluster:
...
EOF
    ...
    # flatten the floraconfig files to make them self contained
    username=$(whoami)
    ${sudo} /usr/bin/env bash -e <<EOF
    $(util::find-binary floractl) --floraconfig="${dest_dir}/${client_id}.floraconfig" config view --minify --flatten > "/tmp/${client_id}.floraconfig"
    mv -f "/tmp/${client_id}.floraconfig" "${dest_dir}/${client_id}.floraconfig"
    chown ${username} "${dest_dir}/${client_id}.floraconfig"
EOF
}
